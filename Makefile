...
test-qeury:
	curl  -H "Authorization: bearer $(GITHUB_TOKEN)" --data @./query-payload.json https://api.github.com/graphql  

github-data.json:
	curl  -H "Authorization: bearer $(GITHUB_TOKEN)" --data @./query-payload.json \
